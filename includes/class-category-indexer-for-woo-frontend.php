...
if ( ! class_exists( 'Category_Indexer_For_Woo_Frontend' ) ) {
	...
	class Category_Indexer_For_Woo_Frontend {
		 ...
		 * that gets added when editing WooCommerce templates.
		 *
		...
		public function enqueue_pagination_fix_script() {
			...
			(function() {
				...
				// Check if URL contains /page/1/, ?paged=1, or ?cst (FSE/Block themes)
				if (url.match(/\/page\/1\/?(\?|#|$)/) || url.match(/[\?&]paged=1(&|$)/) || url.match(/[\?&]cst(&|$)/)) {
					// Clean the URL
